...
export default function HistoryPage() {
  ...
  const gradeColors: Record<string, { bg: string; text: string }> = {
    ...
    'B賞': { bg: 'linear-gradient(135deg, #60a5fa, #2563eb)', text: 'white' },
    'C賞': { bg: 'linear-gradient(135deg, #94a3b8, #64748b)', text: 'white' },
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f5f5f5', paddingBottom: '70px' }}>
      ...
      <div style={{ maxWidth: '900px', margin: '0 auto', padding: '20px 16px' }}>
        <h1 style={{ fontSize: '20px', fontWeight: '800', color: '#1f2937', marginBottom: '20px' }}>当選履歴</h1>
        ...
        {loading ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {draws.map((draw) => {
              const grade = gradeColors[draw.grade] || { bg: '#94a3b8', text: 'white' }
              return (
                <div key={draw.id} style={{ background: 'white', borderRadius: '12px', overflow: 'hidden', border: '1px solid #e5e7eb', display: 'flex', alignItems: 'center', gap: '14px', padding: '14px' }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ marginBottom: '6px' }}>
                      <span style={{ fontSize: '12px', fontWeight: '800', color: grade.text, background: grade.bg, padding: '3px 10px', borderRadius: '999px' }}>{draw.grade}</span>
                    </div>
                    <div style={{ fontSize: '15px', fontWeight: '700', color: '#1f2937', marginBottom: '4px' }}>{draw.products?.name}</div>
                    <div style={{ fontSize: '12px', color: '#6b7280', marginBottom: '6px' }}>{draw.events?.name}</div>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      ...
                      <span style={{ fontSize: '11px', color: '#9ca3af' }}>{formatDate(draw.created_at)}</span>
                    </div>
                  </div>
                </div>
              )
